...
document.getElementById("subjects").innerHTML = courses.map(course => `<li>${course.split(' ')[0]}</li>`);
...
document.getElementById("u").innerHTML = countries.filter(country => country[0].toUpperCase()==='U');
...
document.getElementById("total").innerText = fruits.reduce((accumulator, fruit) => accumulator + fruit.price, 0);
...
document.getElementById("product").textContent = products.find(product => product.price > 20).name;

// *************************************************************************************
// 6️⃣ determine if Canada is contained within the array countries (see line 18)
const idx = countries.indexOf("Canada");
document.getElementById("output").textContent = idx === -1 ? 'not found' : idx;
