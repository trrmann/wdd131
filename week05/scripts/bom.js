...
function removeChapter(event, listItemID) {
    ...
    const chapter = listItem.textContent.slice(0, listItem.textContent.length - 1);
...
function addChapter(event) {
    if (inputElement.value != '') {  // make sure the input is not empty
        displayListElement(inputElement); // call the function that outputs the submitted chapter
...
function displayListElement(inputElement) {
...
function displayList(input) {
    input = bookChapterValidate(input.trim());
    registry.add(input.trim().toLowerCase());
